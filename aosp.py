...
factories_versions = dict((x, sorted([(y[2], tuple(y[:2])) for y in factories if y[3] == x],
                                     key=lambda w: int(w[0].split()[1]))) for x in set((z[3] for z in factories)))
...
def download_and_extract_sdk(version, os, force=False):
    ...
    if force:
        ...
        for _ in (d for d in listdir('.') if isdir(d) and d.startswith('android-sdk_')):
            print 'sdk exists'
            exit(2)
    print 'downloading Android SDK:'
    chunk_read(package, urlopen(url + package), report_hook=chunk_report)
    print 'extracting content'
    zipopen(package).extractall()
    # clean
    remove(package)
...
def _aosp(version, force):
    ...
    for _ in (d for d in listdir('.') if isdir(d) and d == '.hg'):
        break
    else:
        Popen(['hg', 'init'])
...
def aosp_helper():
    ...
    parser.add_option('-v', dest='version', default=versions[-1], help='use a specific version (aosp/sdk/factory)')
